		...
		if (!isSkipAverageCostingQtyCheck) {
			...
			if (ce.isAveragePO() || ce.isAverageInvoice()) 
			{
				if (CurrentQty.signum() < 0) {
				    if (MSysConfig.getValue("Avoid Cost Negative Warning", "N", getAD_Client_ID()).equalsIgnoreCase("Y")) { //ADDED BY ANDI
				        log.warning("Avoid Cost Negative Warning. Product=" 
				            + MProduct.get(getM_Product_ID()).getName()
				            + ", M_Cost_UU=" + getM_Cost_UU()
				            + " set Current Qty=0");
				        CurrentQty = BigDecimal.ZERO;
				    } else {
				        MAcctSchema as = MAcctSchema.get(getC_AcctSchema_ID());
				        throw new AverageCostingNegativeQtyException(
				            "Product=" + MProduct.get(getM_Product_ID()).getName()
				            + ", Current Qty=" + getCurrentQty()
				            + ", New Current Qty=" + CurrentQty
				            + ", CostElement=" + ce.getName()
				            + ", Schema=" + as.getName());
				    }
				}
			}
